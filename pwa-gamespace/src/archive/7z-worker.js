...
class LazyFileContents {
  ...
  subarray(start, end) {
    ...
    this.cache = new Uint8Array(this.reader.readAsArrayBuffer(this.file.slice(safeStart, readEnd)));
    return this.cache.subarray(0, safeEnd - safeStart);
  }
}

function mountLazyArchive(sevenZip, file) {
  ...
  stream.node.contents = new LazyFileContents(file);
