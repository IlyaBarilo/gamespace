...
[Спецификация автономных игр](docs/spec/spec-game.md) ·
...
|---|---|---|
| Платформы | Android 9.0 и новее (проверено на Android 11); iPhone и iPad с необходимыми возможностями браузера | Android 6.0 и новее (проверено на Android 8.1.0) |
...
Установленный сайт работает локально без интернета. В меню APK можно вручную
[проверить и установить обновления приложения](apk-gamespace/APP_UPDATES_RU.md).
...
Обе реализации сначала подготавливают файлы отдельно и используют журнал
операции с откатом, чтобы ошибка, закрытие приложения или нехватка места не
повредили ранее установленный сайт.

Для разработки самостоятельных HTML-игр вручную или с помощью ИИ используйте
[спецификацию автономных игр](docs/spec/spec-game.md). Её можно прикладывать к
запросу как технический контракт и чек-лист совместимости.
...
Допустимы две основные структуры. Можно заархивировать содержимое каталога:
...
файл `.html`. Переход в единственный каталог разрешён только тогда, когда в
корне нет обычных файлов; внутри действует тот же порядок. Поиск не выполняется
рекурсивно.
...
Для полной установки PWA и APK временно требуется место под действующую и новую
ревизии сайта вместе с резервом. Перед применением update-архива обе версии
отдельно учитывают резервные копии фактически заменяемых файлов. Поэтому большие архивы нужно
