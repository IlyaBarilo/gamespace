...
установку за 2 минуты 32 секунды. Значения 2,47 ГБ в PWA и 2,5 ГБ в APK
отличаются только способом отображения и округления объёма; содержимое архива
одно и то же.
...
При каждом push отдельный workflow проверки собирает PWA на Linux и тестовый
APK на Windows, выполняет тесты, проверяет неизменяемый Service Worker и
формирование пакета выпуска. Этот workflow ничего не публикует и не использует
релизный ключ APK. Локальный коммит запускает проверку только после отправки в
GitHub.
